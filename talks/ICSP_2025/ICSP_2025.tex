...
\titlegraphic{
  \vskip -2cm
  \hspace{10cm}
  \uncover<2>{\includegraphics[width=3.5cm]{figures/Joaquim.jpeg}}
}
...
\begin{frame}{Back to the 2-stage problem}
  We adopt a notation where $x$ is the incoming state, $y$ the decision, and $z$ the outgoing state:
  \[
...
\begin{frame}{Some observations}
  ...
  \begin{enumerate}[<+->]
    \item We can form a recursion for value functions:
  \begin{align*}
...
\begin{frame}{More general decision rules}
  ...
  \begin{itemize}
    ...
    \pause
    \item $L(x)$ is a \emph{piecewise-linear} function, so we might need to \emph{convexify} $V$.
    \item The (natural) convexification leads to a valid upper bound for $V_{LDR}(x)$, so for $V(x)$.
\end{itemize}
\end{frame}
