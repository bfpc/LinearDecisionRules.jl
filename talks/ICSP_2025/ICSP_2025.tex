...
\titlegraphic{
  \vskip -2cm
  \hspace{10cm}
  \includegraphics<2>[width=3.5cm]{figures/Joaquim.jpeg}
}
...
\begin{frame}{Some observations}
  \begin{itemize}
    \item In time-decomposition, the decisions $y_t$ (and $x_t$) depend on the previous state $x_{t-1}$ and the uncertainty $\xi_t$;
    \item We could posit $y_t = Y_{t,\xi} \cdot \xi_t + Y_{t,x} \cdot x_{t-1}$, where $Y_{t,\xi}$ and $Y_{t,x}$ are decision rule matrices;\pause
    \item However, recursively substituting a decision rule for $x_{t-1}$ leads to a \emph{product} of decision rule matrices, which is not linear in the uncertainties.
  \end{itemize}
  \pause

  We can form a recursion for ``LDR''-Value functions:
  \[
  \min \mathbb{E}_{\xi_t}\left[ c_t(\xi_t)^\top y_t \cdot \xi_t + V_{t+1}(x_t) \right],
  \]
  because $V_{t+1}$ is convex and $y_t$ and $x_t$ are linear in the decision rule matrices ($Y_{t,\xi}$, $Y_{t,x}$, $X_{t,\xi}$, $X_{t,x}$).
\end{frame}

\begin{frame}{Back to the 2-stage problem}
  We adopt a notation for $x$ the incoming state, $y$ the decision, and $z$ the outgoing state:
  \[
    V(x) = \begin{array}[t]{rl}
      \min        & \mathbb{E}\left[ c(\xi)^\top y(\xi) + f(z(\xi)) \right] \\[0.5ex]
      \text{s.t.} & T x + W_y y(\xi) + W_z z(\xi) = h(\xi) \\
      & y(\xi), z(\xi) \geq 0.
    \end{array}
  \]
  \pause
  ...
  For fixed $x$, the LFDR is a feasible LDR for both $y$ and $z$, so $V_{LFDR}(x) \geq V_{LDR}(x) \geq V(x)$.
\end{frame}

\begin{frame}{However}
  Such a policy is too simplistic, leading to $\frac{\partial V_{LFDR}(x)}{\partial x}$ being constant \alert{VERIFY}.
  Therefore
  \begin{itemize}
    ...
    \item If $x = \sum x_i$, this leads to a piecewise-linear decision rule.
  \end{itemize}

  Now, we have $V_{PWLF}(x) = \mathbb{E}\left[ c(\xi)^\top \hat{Y}_\xi \cdot \xi + c_x^\top \hat{Y}_x \cdot L(x) \right]$.
  However, $L(x)$ is a \emph{piecewise-linear} function, so we might need to \emph{convexify} $V$ to ensure the recursion remains manageable.
  But, again, the PWLF is a feasible LDR, so $V_{PWLF}(x) \geq V_{LDR}(x) \geq V(x)$, and the former are feasible, so the (natural) convexification leads to a valid upper bound for $V(x)$.
\end{frame}
...
\begin{frame}{A toy example}
  \centering \includegraphics[height=0.8\textheight]{figures/1dtoy_value_function_comparison.pdf}
\end{frame}
...
\begin{frame}{Slightly more complex example}
  \centering \includegraphics[width=0.45\textwidth]{figures/1d_vf_23.pdf} \includegraphics[width=0.45\textwidth]{figures/1d_water_value_23.pdf}
\end{frame}

\begin{frame}{Slightly more complex example}
  \centering \includegraphics[width=0.45\textwidth]{figures/1d_vf_22.pdf} \includegraphics[width=0.45\textwidth]{figures/1d_water_value_22.pdf}
\end{frame}

\begin{frame}{Slightly more complex example}
  \centering \includegraphics[width=0.45\textwidth]{figures/1d_vf_2.pdf} \includegraphics[width=0.45\textwidth]{figures/1d_water_value_2.pdf}
\end{frame}
