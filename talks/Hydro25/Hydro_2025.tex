...
\begin{frame}{Linear Decision Rules --- Reformulation}
  If the uncertainty set $\Xi$ is given by linear constraints $\{\, \xi : W \xi \geq h \,\}$, we can rewrite the optimization problem as a \emph{deterministic} linear program over the decision rule matrix $X$ and auxiliary variables $\Lambda$ (for the positivity constraints):
  \[ \begin{array}{rl}
